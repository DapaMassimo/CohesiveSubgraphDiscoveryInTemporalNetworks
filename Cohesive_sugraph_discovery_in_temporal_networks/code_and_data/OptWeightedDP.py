...
def OptWeightedDP(k, timestamps):
    ...
    for i in range(0,m):
        ...
        DP[0,i] = round(get_solution(g)[1]/(i+1), 8)
        C[0,i] = 0
    ...
    for l in range(1, k):
        ...
        for i in range(l, m):
            ...
            for j in range(l-1, i):
                ...
                g = nx.Graph()
                g.add_weighted_edges_from(edge_freq)
                choice = DP[l-1, j] + get_solution(g)[1]/(i-(j+1)+1)
...
#%%
def OptWeightedDP_keep_going(dp, c, k_left, timestamps):
    """
